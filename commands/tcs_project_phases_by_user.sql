SELECT {+AVOID_FULL(p), AVOID_FULL(pp), AVOID_FULL(d)} 
      pp.project_phase_id, pp.project_id, pp.fixed_start_time,
...
AND p.project_id in (select {+AVOID_FULL(r)} r.project_id from resource r where r.user_id = @uid@)