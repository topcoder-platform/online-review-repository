...
public class ProjectService extends ProjectServiceGrpc.ProjectServiceImplBase {
    ...
    @Override
    public void xCreateProject(XCreateProjectRequest request, StreamObserver<XProjectProto> responseObserver) {
        ...
        XProjectProto.Builder pBuilder = createProject(request.getProject(), request.getOperator());
        if (request.getProject().getPropertiesCount() > 0) {
    ...
    @Override
    public void xUpdateProject(XUpdateProjectRequest request, StreamObserver<XProjectProto> responseObserver) {
        Map<String, Long> propertyTypeNameIdMap = new HashMap<>();
        if (request.getProject().getPropertiesCount() > 0) {
            propertyTypeNameIdMap = makePropertyNamePropertyIdMap(getAllProjectPropertyTypes());
        }
        validateUpdateProjectRequest(request, propertyTypeNameIdMap);
        XProjectProto.Builder pBuilder = updateProject(request.getProject(), request.getReason(),
                request.getOperator());
        if (request.getProject().getPropertiesCount() > 0) {
    ...
    public void getProjectFileTypes(ProjectIdProto request,
        ...
        String sql = """
                ...
                """;
        List<XFileTypeProto> result = dbAccessor.executeQuery(sql, (rs, _i) -> {
            XFileTypeProto.Builder builder = XFileTypeProto.newBuilder();
    ...
    public void updateProjectFileTypes(UpdateProjectFileTypesRequest request,
        ...
        List<XFileTypeProto> fileTypes = updateProjectFileTypes(request.getProjectId(), request.getFileTypesList(),
                request.getOperator());
    ...
    public void getProjectPrizes(ProjectIdProto request, StreamObserver<GetProjectPrizesResponse> responseObserver) {
        ...
        String sql = """
                ...
                """;
        List<XPrizeProto> result = dbAccessor.executeQuery(sql, (rs, _i) -> {
            XPrizeProto.Builder builder = XPrizeProto.newBuilder();
            ResultSetHelper.applyResultSetLong(rs, 1, builder::setId);
            ...
            XPrizeTypeProto.Builder ptBuilder = XPrizeTypeProto.newBuilder();
    ...
    public void updateProjectPrizes(UpdateProjectPrizesRequest request,
        ...
        List<XPrizeProto> prizes = updateProjectPrizes(request.getProjectId(), request.getPrizesList(),
                request.getOperator());
    ...
    @Override
    public void createFileType(CreateFileTypeRequest request, StreamObserver<XFileTypeProto> responseObserver) {
        validateCreateFileTypeRequest(request);
        XFileTypeProto fileType = createFileType(request.getFileType(), request.getOperator());
        responseObserver.onNext(fileType);
        responseObserver.onCompleted();
    }

    @Override
    public void updateFileType(UpdateFileTypeRequest request, StreamObserver<XFileTypeProto> responseObserver) {
        ...
        String sql = """
                ...
                WHERE file_type_id = ?
                """;
        XFileTypeProto fileType = request.getFileType();
        dbAccessor.executeUpdate(sql, fileType.getDescription(), fileType.getSort(),
                ...
                now);
        fileType = XFileTypeProto.newBuilder(fileType).setModifyUser(request.getOperator()).setModifyDate(nowTs)
                .build();
        responseObserver.onNext(fileType);
        responseObserver.onCompleted();
    }
    ...
    public void getAllFileTypes(Empty request, StreamObserver<GetAllFileTypesResponse> responseObserver) {
        String sql = """
                ...
                """;
        List<XFileTypeProto> fileTypes = dbAccessor.executeQuery(sql, (rs, _id) -> {
            XFileTypeProto.Builder builder = XFileTypeProto.newBuilder();
            ResultSetHelper.applyResultSetLong(rs, 1, builder::setId);
            ResultSetHelper.applyResultSetString(rs, 2, builder::setDescription);
            ResultSetHelper.applyResultSetInt(rs, 3, builder::setSort);
    ...
    public void getAllPrizeTypes(Empty request, StreamObserver<GetAllPrizeTypesResponse> responseObserver) {
        String sql = """
                ...
                """;
        List<XPrizeTypeProto> prizeTypes = dbAccessor.executeQuery(sql, (rs, _id) -> {
            XPrizeTypeProto.Builder builder = XPrizeTypeProto.newBuilder();
            ResultSetHelper.applyResultSetLong(rs, 1, builder::setId);
            ResultSetHelper.applyResultSetString(rs, 2, builder::setDescription);
            return builder.build();
        });
    ...
    @Override
    public void createPrize(CreatePrizeRequest request, StreamObserver<XPrizeProto> responseObserver) {
        validateCreatePrizeRequest(request);
        XPrizeProto prize = createPrize(request.getPrize().getProjectId(), request.getPrize(), request.getOperator());
        responseObserver.onNext(prize);
        responseObserver.onCompleted();
    }

    @Override
    public void updatePrize(UpdatePrizeRequest request, StreamObserver<XPrizeProto> responseObserver) {
        validateUpdatePrizeRequest(request);
        XPrizeProto prize = updatePrize(request.getPrize().getId(), request.getPrize(), request.getOperator());
        responseObserver.onNext(prize);
        responseObserver.onCompleted();
    }
    ...
    public void createStudioSpec(CreateStudioSpecRequest request,
            StreamObserver<XProjectStudioSpecProto> responseObserver) {
        validateCreateStudioSpecRequest(request);
        XProjectStudioSpecProto spec = createStudioSpec(request.getProjectStudioSpec(), request.getOperator());
    ...
    public void updateStudioSpec(UpdateStudioSpecRequest request,
        ...
        XProjectStudioSpecProto spec = updateStudioSpec(request.getProjectStudioSpec(), request.getOperator());
        responseObserver.onNext(spec);
        responseObserver.onCompleted();
    }
    ...
    public void getProjectStudioSpec(ProjectIdProto request,
        ...
        String sql = """
                ...
                """;
        List<XProjectStudioSpecProto> result = dbAccessor.executeQuery(sql, (rs, _i) -> {
            XProjectStudioSpecProto.Builder builder = XProjectStudioSpecProto.newBuilder();
            ResultSetHelper.applyResultSetLong(rs, 1, builder::setId);
        ...
        if (result.isEmpty()) {
            ...
            response = GetProjectStudioSpecResponse.newBuilder().setProjectStudioSpec(result.get(0)).build();
        }
    ...
    public void updateProjectStudioSpec(UpdateProjectStudioSpecRequest request,
        ...
        XProjectStudioSpecProto spec = updateProjectStudioSpec(request.getProjectId(), request.getProjectStudioSpec(),
                request.getOperator());
        responseObserver.onNext(spec);
        responseObserver.onCompleted();
    }
    ...
    public void isProjectExists(IdProto request, StreamObserver<ExistsProto> responseObserver) {
        validateIdProto(request);
        boolean exists = isProjectExists(request.getId());
    ...
    public void isFileTypeExists(IdProto request, StreamObserver<ExistsProto> responseObserver) {
        validateIdProto(request);
        boolean exists = isFileTypeExists(request.getId());
        responseObserver.onNext(ExistsProto.newBuilder().setExists(exists).build());
        responseObserver.onCompleted();
    }
    ...
    public void isPrizeExists(IdProto request, StreamObserver<ExistsProto> responseObserver) {
        validateIdProto(request);
        boolean exists = isPrizeExists(request.getId());
        responseObserver.onNext(ExistsProto.newBuilder().setExists(exists).build());
        responseObserver.onCompleted();
    }
    ...
    public void isStudioSpecExists(IdProto request, StreamObserver<ExistsProto> responseObserver) {
        validateIdProto(request);
        boolean exists = isStudioSpecExists(request.getId());
        responseObserver.onNext(ExistsProto.newBuilder().setExists(exists).build());
        responseObserver.onCompleted();
    }

    private XProjectProto.Builder createProject(XProjectProto project, String operator) {
        Long newId = 0L;
        Date now = new Date();
        Timestamp nowTs = Timestamp.newBuilder().setSeconds(now.toInstant().getEpochSecond()).build();
        String sql = """
        ...
        return XProjectProto.newBuilder(project).setId(newId).setCreateUser(operator).setCreateDate(nowTs)
                .setModifyUser(operator).setModifyDate(nowTs);
    }

    private XProjectProto.Builder updateProject(XProjectProto project, String reason, String operator) {
        Date now = new Date();
        Timestamp nowTs = Timestamp.newBuilder().setSeconds(now.toInstant().getEpochSecond()).build();
        String sql = """
        ...
        return XProjectProto.newBuilder(project).setModifyUser(operator).setModifyDate(nowTs);
    }

    private void createProjectProperties(long projectId, String operator, List<XProjectPropertyProto> properties,
            Map<String, Long> nameIdMap) {
        if (properties.isEmpty()) {
            return;
        }
        for (XProjectPropertyProto property : properties) {
    ...
    private void updateProjectProperties(long projectId, String operator, List<XProjectPropertyProto> properties,
            Map<String, Long> nameIdMap) {
    ...
    private List<XProjectPropertyProto> getProjectPropertyIdValues(long projectId) {
        String sql = """
                SELECT project_info_type_id, value
                FROM project_info
                WHERE project_id = ?
        ...
        return dbAccessor.executeQuery(sql, (rs, _i) -> {
            XProjectPropertyProto.Builder builder = XProjectPropertyProto.newBuilder();
            ResultSetHelper.applyResultSetLong(rs, 1, builder::setId);
            ResultSetHelper.applyResultSetString(rs, 2, builder::setValue);
            return builder.build();
        }, projectId);
    ...
    private List<XFileTypeProto> createProjectFileTypes(long projectId, List<XFileTypeProto> fileTypes,
            String operator) {
        List<XFileTypeProto> newFileTypes = new ArrayList<>();
        for (XFileTypeProto fileType : fileTypes) {
            long id = fileType.getId();
            if (id == 0) {
                XFileTypeProto created = createFileType(fileType, operator);
                id = created.getId();
    ...
    private List<XFileTypeProto> updateProjectFileTypes(long projectId, List<XFileTypeProto> fileTypes,
            String operator) {
    ...
    private XFileTypeProto createFileType(XFileTypeProto fileType, String operator) {
        Long newId = 0L;
        Date now = new Date();
        Timestamp nowTs = Timestamp.newBuilder().setSeconds(now.toInstant().getEpochSecond()).build();
        String sql = """
        ...
        return XFileTypeProto.newBuilder(fileType).setId(newId).setCreateUser(operator).setCreateDate(nowTs)
    ...
    private List<XPrizeProto> updateProjectPrizes(long projectId, List<XPrizeProto> prizes, String operator) {
        List<XPrizeProto> newPrizes = new ArrayList<>();
        for (XPrizeProto prize : prizes) {
            if (prize.getId() == 0) {
    ...
    private List<XPrizeProto> createProjectPrizes(long projectId, List<XPrizeProto> prizes, String operator) {
        List<XPrizeProto> newPrizes = new ArrayList<>();
        for (XPrizeProto prize : prizes) {
            newPrizes.add(createPrize(projectId, prize, operator));
        }
    ...
    private XPrizeProto createPrize(long projectId, XPrizeProto prize, String operator) {
        Long newId = 0L;
        Date now = new Date();
        Timestamp nowTs = Timestamp.newBuilder().setSeconds(now.toInstant().getEpochSecond()).build();
        String sql = """
        ...
        return XPrizeProto.newBuilder(prize).setId(newId).setProjectId(projectId).setCreateUser(operator)
                .setCreateDate(nowTs).setModifyUser(operator).setModifyDate(nowTs).build();
    }

    private XPrizeProto updatePrize(long projectId, XPrizeProto prize, String operator) {
        Date now = new Date();
        Timestamp nowTs = Timestamp.newBuilder().setSeconds(now.toInstant().getEpochSecond()).build();
        String sql = """
        ...
        return XPrizeProto.newBuilder(prize).setProjectId(projectId).setModifyUser(operator).setModifyDate(nowTs)
                .build();
    }

    private XProjectStudioSpecProto createProjectStudioSpec(long projectId, XProjectStudioSpecProto studioSpec,
            String operator) {
        XProjectStudioSpecProto created = createStudioSpec(studioSpec, operator);
        String sql = """
                UPDATE project
    ...
    private XProjectStudioSpecProto updateProjectStudioSpec(long projectId, XProjectStudioSpecProto studioSpec,
            String operator) {
    ...
    private XProjectStudioSpecProto createStudioSpec(XProjectStudioSpecProto studioSpec, String operator) {
        Long newId = 0L;
        Date now = new Date();
        Timestamp nowTs = Timestamp.newBuilder().setSeconds(now.toInstant().getEpochSecond()).build();
        String sql = """
        ...
        return XProjectStudioSpecProto.newBuilder(studioSpec).setId(newId).setCreateUser(operator).setCreateDate(nowTs)
                .setModifyUser(operator).setModifyDate(nowTs).build();
    }

    private XProjectStudioSpecProto updateStudioSpec(XProjectStudioSpecProto studioSpec, String operator) {
        Date now = new Date();
        Timestamp nowTs = Timestamp.newBuilder().setSeconds(now.toInstant().getEpochSecond()).build();
        String sql = """
        ...
        return XProjectStudioSpecProto.newBuilder(studioSpec).setModifyUser(operator).setModifyDate(nowTs).build();
    }
    ...
    private Map<Long, String> makePropertyIdPropertyValueMap(List<XProjectPropertyProto> properties) {
        return properties.stream()
                .collect(Collectors.toMap(XProjectPropertyProto::getId, XProjectPropertyProto::getValue));
    }
    ...
    private void validateCreateProjectRequest(XCreateProjectRequest request, Map<String, Long> propertyTypeNameIdMap) {
        Helper.assertObjectNotNull(request::hasProject, "project");
        Helper.assertObjectNotNull(request::hasOperator, "operator");
        XProjectProto project = request.getProject();
        ...
        for (XProjectPropertyProto property : project.getPropertiesList()) {
        ...
        for (XPrizeProto prize : project.getPrizesList()) {
            validateProjectPrize(prize);
        }
    }

    private void validateUpdateProjectRequest(XUpdateProjectRequest request, Map<String, Long> propertyTypeNameIdMap) {
        Helper.assertObjectNotNull(request::hasProject, "project");
    ...
    private void validateProjectFileTypes(List<XFileTypeProto> fileTypes) {
        for (XFileTypeProto fileType : fileTypes) {
            if (fileType.getId() > 0) {
    ...
    private void validateUpdateProjectPrizesRequest(UpdateProjectPrizesRequest request) {
        ...
        for (XPrizeProto prize : request.getPrizesList()) {
            if (prize.getId() > 0) {
                if (!isPrizeExists(prize.getId())) {
                    throw new IllegalArgumentException(
                            "Prize with id '%s' cannot be found".formatted(prize.getId()));
    ...
    private void validateFileType(XFileTypeProto fileType) {
    ...
    private void validateProjectPrize(XPrizeProto prize) {
    ...
    private void validateCreatePrizeRequest(CreatePrizeRequest request) {
        XPrizeProto prize = request.getPrize();
        Helper.assertObjectNotNull(request::hasOperator, "operator");
