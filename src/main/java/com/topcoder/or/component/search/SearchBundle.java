...
import com.topcoder.or.component.datavalidator.NotValidator;
import com.topcoder.or.component.datavalidator.ObjectValidator;
import com.topcoder.or.component.search.filter.AndFilter;
import com.topcoder.or.component.search.filter.BetweenFilter;
import com.topcoder.or.component.search.filter.EqualToFilter;
import com.topcoder.or.component.search.filter.Filter;
import com.topcoder.or.component.search.filter.GreaterThanFilter;
import com.topcoder.or.component.search.filter.GreaterThanOrEqualToFilter;
import com.topcoder.or.component.search.filter.InFilter;
import com.topcoder.or.component.search.filter.LessThanFilter;
import com.topcoder.or.component.search.filter.LessThanOrEqualToFilter;
import com.topcoder.or.component.search.filter.LikeFilter;
import com.topcoder.or.component.search.filter.NotFilter;
import com.topcoder.or.component.search.filter.OrFilter;
 ...
 * component.It can
 * Build Filter tree. Since the Filter package employs the composite pattern, we
 * have simple Filters
 * and associative filters. Thus all Filters defined for a search can form a
 * tree. The leaves are
 * simple Filters and the parent nodes are associative Filters.And also thoses
 * filters can be
 ...
 * ObjectValidator
 * objects.Also it do the converting the Filter to search string via the
 * builder.
 *
 * <p>
 * Most import, all the searchFunction is implemented by this class as the API
 * of the component.
...
public class SearchBundle {
  ...
  /**
   * It will hold a Map of ObjectValidator objects. keys are the real name (or
   * alias name) of
  ...
  /**
   * It will hold a Map containing keys as the alias names of the fields and the
   * names of fileds as
   * stored objects. The search Fields name in the filter can both be the alias
   * name and real name.
  ...
  /**
   * This is the search strategy that is used to perform the actual searching work
   * for the bundle.
   * The search strategy is responsible for connecting to the datastore, building
   * the necessary
   * query String (if applicable) and executing it against the datastore. The
   * object returned from
   * the search is dependent on the type of datastore that is used. It is added in
   * version 1.3.
   */
  private SearchStrategy searchStrategy;

  /**
   * Create a new instance,providing the name ,searchable fields,map of alias. All
   * the other member
   * are initialized to null.
   *
   * @param name    name of the search bundle
   * @param fields  a map of searchable fields
   * @param alias   a map of strings, holding the names of fields as stored
   *                objects and alias names as
   *                keys
   * @param context the context of the search bundle
   * @throws IllegalArgumentException if any parameter is null or the keys and
   *                                  values in the map is
   *                                  null, or name, or fileds, or context is
   *                                  empty
   */
  ...
  /**
   * Create a new instance,providing the name ,map of alias. All the other member
   * are initialized to
   * null.
   *
   * @param name    name of the search bundle
   ...
   * datastore. The
   * keys of the fields are the names of the fields which are searchable. The
   * values of the fields
   * are the ObjectValidator instances which may be used to constrain the valid
   * values that are used
   * for the filters. An empty fields means that ALL the fields in the context are
   * searchable and no
   * validation will need to occur. A field mapped to a null value means that
   * there are no
   ...
   * (for example, the
   * column widgets.widget_name may be better off being named "Widget Name"). The
   * values will
   * contain the search field as it would appear in the datastore context.
   *
   * @param name           name of the search bundle
   * @param fields         a map of searchable fields
   * @param alias          a map of strings, holding the alternate names of fields
   *                       as keys and their actual
   *                       values in the datastore as the respective values.
   * @param context        the context of the search bundle
   * @param searchStrategy The search Strategy to use.
   * @throws IllegalArgumentException if any parameter is null or name, or fileds
   *                                  is empty, or the
   *                                  alias is invalid
   */
  public SearchBundle(
      String name, Map<String, NotValidator> fields, Map<String, String> alias, String context,
  ...
  /**
   * Execut the search with given constructed Filter,both simple and
   * AssociativeFilter will do. The
   * result will be returned as an object instanceof CustomResultSet for the
   * DataBase search. The
   * result will be returned as an abject instanceof Iterator of the Entry for the
   * LDAP.
   *
   ...
   * @throws PersistenceOperationException if any error when operating over data
   *                                       store or the filter
   *                                       is invalid
   */
  public <T> List<T> search(Filter filter, RowMapper<T> mapper) throws SearchBuilderException {
    return search(filter, new ArrayList(), mapper);
  }

  /**
   * Execut the search with given constructed Filter and return fileds, now the
   * return fields are
   * not only about the context,but also ref to returnFields,both will be
   * returned. The result will
   * be returned as an object instanceof CustomResultSet for the DataBase search.
   * The result will be
   ...
   *                                       is invalid.
   */
  public <T> List<T> search(Filter filter, List returnFields, RowMapper<T> mapper)
    ...
    try {
      synchronized (this) {
        return searchStrategy.search(context, filter, returnFields, aliasMap, mapper);
      }
    } catch (Exception e) {
      // wrap the PersistenceOperationException if error occurs during the
      // validateFilter
   ...
   * @param value a Comparable object representing the value constraint of the
   *              field
   ...
   *
   * @param name  name of the field, might be alias, mihgt be real name
   * @param upper a Comparable object representing the upper threshold value of
   *              the field
   * @param lower a Comparable object representing the lower threshold value of
   *              the field
   * @return constructed Filter object
   * @throws IllegalArgumentException if any parameter is null
   * @throws IllegalArgumentException if name is an empty string
   ...
   *
   * @param name   name of the field, might be alias, mihgt be real name
   * @param values a Comparable object representing the value constraints of the
   *               field
   * @return constructed Filter object
   * @throws IllegalArgumentException if any parameter is null
   * @throws IllegalArgumentException if name is an empty string
   ...
   * @throws IllegalArgumentException if any parameter is null or empty, or value
   *                                  does not begin
   *                                  with 'SS', 'SW', 'EW', 'WC', or the value
   *                                  without the prefix is zero length
   * @deprecated This method is now deprecated.
   */
   ...
   * This method is added in version 1.2.
   *
   * @param name            the name of the field, might be alias, mihgt be real
   *                        name
   * @param value           the searching value in the filter
   ...
   *                                  does not begin
   *                                  with 'SS', 'SW', 'EW', 'WC', or the value
   *                                  without the prefix is zero length, or the
   *                                  escapeCharacter is of some masked chars('*',
   *                                  '%', '_')
   * @deprecated This method is now deprecated.
   */
   ...
   * are the keys while the ObjectValidators are the values. Also the keys may
   * both the realname and
   * alias name.
   *
  ...
  /**
   * Retrieves the search strategy that is used to perform the actual searching
   * work for the bundle.
   * The search strategy is responsible for connecting to the datastore, building
   * the necessary
   * query String (if applicable) and executing it against the datastore. The
   ...
   *
   * @return the search strategy that is used to perform the actual searching work
   *         for the bundle.
   */
  ...
  /**
   * Sets the search strategy that is used to perform the actual searching work
   * for the bundle. The
   * search strategy is responsible for connecting to the datastore, building the
   * necessary query
   * String (if applicable) and executing it against the datastore. The object
   * returned from the
   * search is dependent on the type of datastore that is used.
   *
   * @param searchStrategy the search strategy that is used to perform the actual
   *                       searching work for
   *                       the bundle.
   ...
   * The mothed also
   * check the key and value in the map which also should not be null. More, both
   * the key and value
   * should String type.
   *
   * <p>
   ...
   * @throws IllegalArgumentException if the map is null, if the keys and values
   *                                  illegal
   */
  ...
  /**
   * Check the fieldsMap valid.The map should not be null also should not be
   * empty. The mothed also
   * check the key and value in the map which also should not be null. More, key
   * should be String
   ...
   * @param map     The map to be checked
   * @param mapName the Name of the map, which denote the usage of the map
   * @throws IllegalArgumentException if the map is null, or the map is empty or
   *                                  the keys and values
   *                                  illegal
   */
