 ...
 * Search. It contains
 * any bindable parameters, as well as the existing search string. It may be
 * used by any
 * SearchStrategies as a convenience class to help with building the search
 * string and relevant
 * parameters.
 *
 * <p>
 * Thread Safety: This class is not required to be thread safe. Only one
 * SearchContext will be
 * used per thread.
 *
...
public class SearchContext {
   ...
   * empty, but may be
   * built by direct access to the StringBuilder class via
   * <code>getSearchString</code>.
   */
  private final StringBuffer searchString;

  /**
   * This is a list of bindableParameters that have been accumulated during the
   * Search Building
   * process. It is initally empty, but may be modified by direct access to the
   * bindableParameters
   ...
   * names of fileds as
   * stored objects. After the initialization, it can not be null, it will be
   * initialized in the
   * constructor. It is immutable. Value will not be changed during the life time
   * of the instance
   */
   ...
   *
   * <p>
   * fragmentBuilderAssociations may be null, to indicate that the SearchContext
   * does not need to
   * provide lookups.
   *
   * @param aliasMap a map of strings, holding the alternate names of fields as
   *                 keys and their
   *                 actual values in the datastore as the respective values.
   * @throws IllegalArgumentException if the aliasMap is null, or aliasMap
   *                                  contains null or empty
   *                                  String elements or non-string elements for
   *                                  both key and value.
   */
  ...
  /**
   * Retrieves a list of bindableParameters that have been accumulated during the
   * Search Building
   * process.
   *
   * <p>
   ...
   * paramters.
   *
   * @return a list of bindableParameters that have been accumulated during the
   *         Search Building
   *         process.
   */
   ...
   * filter. If the
   * fragmentBuilders is null, or if no association can be found, then a null is
   * returned.
   *
   ...
   *
   * @return The actual field name that was represented by the alias. A null is
   *         returned if the
   *         alias could not be recognized.
   * @param alias An alias that is used as a convenient form of representing the
   *              actual field name.
