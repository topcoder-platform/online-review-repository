...
import com.topcoder.or.component.search.builder.SearchBuilderHelper;
import com.topcoder.or.component.search.builder.SearchFragmentBuilder;
 ...
 * the necessary SQL search string appropriate to the filters provided and
 * executing the SQL against
 * the database. This is done with the help of the SearchFragmentBuilder
 * implementations that are
 * provided in this package. Each SearchFragmentBuilder is responsible for
 * building the SQL for a
 * specific filter, and a ClassAssociator is used to associate the
 * FragmentBuilders with the filters
...
public class SearchStrategy {
  ...
  public SearchStrategy(DBAccessor dbAccessor) {
    this.dbAccessor = dbAccessor;
  }

  /**
   * This is a protected method that can be used to build the SearchContext for
   * the search. It may
   * be overridden by subclasses to provide additional or different methods of
   * building up the
   * search context.
   *
   * <p>
   * In this Strategy, the SearchContext is used to hold the state of processing
   * the Filters. The
   * search String is built separately from the binding parameters. The SQL
   * SearchFragmentBuilders
   * are expected to do the following: 1. Update the searchString by appending the
   * necessary String
   * fragment relevant to the Filter. Usually this is a String fragment that
   * contains a
   * PreparedStatement-bindable parameter. For example: "a > ?" 2. If a bindable
   * parameter(s) were
   * added, append them to the Bindable parameters of the SearchContext.
   *
   * <p>
   * This method may be overridden by subclasses to add additional behavior (for
   * example,
   * processing the SQL String to selectively choose the table joins for increased
   * query
   * performance). For such a method, all it would do is call
   * super.buildSearchContext, then modify
   ...
   * @param filter       The search filter which will constrain the search
   *                     results.
   * @param returnFields A list of return fields to further constrain the search
   *                     an empty List means
   *                     that all fields in the context should be returned by the
   *                     search.
   * @param aliasMap     a map of strings, holding the alternate names of fields
   *                     as keys and their
  ...
  /**
   * Searches the database using the provided context, filter and constraints the
   * returnFields.
   *
   ...
   *                     actual values in the datastore as the respective values.
   * @throws UnrecognizedFilterException   propagated from SearchFragmentBuilder
   * @throws IllegalArgumentException      if context or filter is null or if
   *                                       context is an empty String
   *                                       or if returnFields contains
   *                                       null/non-String contents or aliasMap
   *                                       contains null/empty String
   *                                       arguments.
   * @throws PersistenceOperationException to wrap any exception that occurs while
   *                                       searching (except
   *                                       UnrecognizedFilterException and IAE).
   */
  public <T> List<T> search(
      String context, Filter filter, List returnFields, Map aliasMap, RowMapper<T> mapper)
    ...
    try {
      return dbAccessor.executeQuery(searchContext.getSearchString().toString(), mapper,
          searchContext.getBindableParameters().toArray());
    } catch (Exception e) {
      e.printStackTrace();
  ...
  /**
   * Build a complete search string via including return fields into the search
   * string at last,the
   * return fields will include both of those in the list and the searchString.
   * The mothed do not
   * need synchronized.
   *
