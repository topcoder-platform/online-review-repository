...
/**
 * <p>
 * This is an abstract <code>ObjectValidator</code> implementation which gives
 * the ability for resource bundled message
 * fetching for the validation messages.
 * </p>
 *
 * <p>
 * The user will be able to set up which bundle should be used and what key
 * should be used to fetch the message. We
 * also have the ability to specify the locale. But if one is not provided then
 * the current locale will be used. Note
 * that the bundle functionality is fail safe in the sense the user must provide
 * a default message in case the bundle
 * functionality fails. This way the validator is always guaranteed to be able
 * to produce a message. The user is not
 * required though to utilize the resource bundle functionality and can directly
 * supply just the message that will be
 * then used by the implementing validator.
 * </p>
 *
 * <p>
 * <b>Thread Safety:</b>This is a thread-safe implementation since it is
 * immutable.
 * </p>
 *
...
public abstract class AbstractObjectValidator implements ObjectValidator {
     ...
     * validator amongst other validators in a
     * composite validator for example. This is set through a dedicated setter and
     * is mutable. We can also read it
     * through a dedicated getter. Access to this must be synchronized for
     * thread-safety. It's initialized as an empty
    ...
    /**
     * <p>
     * This represents the resource bundle information used by this validator. It is
     * set in one of the constructors to
     * either null (not required) or to a specific named bundle. This is then used
     * to fetch the specific message based
     * on a resource key (which is also initialized in the same constructor) It is
     * transient since it is not
     * serializable. It can be changed upon serialization and subsequent
     * de-serialization.
     * </p>
     */
    ...
    /**
     * <p>
     * This is a simple constructor which initializes the
     * <code>defaultMessage</code> to the input string.
     * </p>
     *
     * @param validationMessage String This is the validation message to use for the
    ...
    /**
     * <p>
     * This is a simple constructor which initializes all the internal variables to
     ...
     *
     * @throws IllegalArgumentException if the <code>bundleInfo</code> is null, or
     *                                  if the <code>bundleName</code>,
     *                                  <code>defaultMessage</code>,
     *                                  <code>locale</code> or
     *                                  <code>resourceBundle</code> of
     *                                  <code>bundleInfo</code> is null, or empty
     ...
     * for this validator. This will be
     * either the message obtained from resource bundle or a default message. Please
     * note that this could return null.
     * </p>
     *
    ...
    /**
     * <p>
     * Checking the <code>BundleInfo</code> to ensure that enough information was
     * provided.
     * </p>
     *
     ...
     * is composite. Since this is a
     * primitive validator the result will always be equivalent to getting a simple
     ...
     * validator the result will always be
     ...
     * accepts a limit on how many messages
     * at most will be requested. This means that the traversal of the validator
     * tree will stop the moment
