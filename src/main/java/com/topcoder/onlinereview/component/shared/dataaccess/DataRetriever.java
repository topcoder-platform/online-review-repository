...
import static com.topcoder.or.util.Helper.getInt;
import static com.topcoder.or.util.Helper.getLong;
import static com.topcoder.or.util.Helper.getString;
 ...
 *
 * <ul>
 * <li><strong>data_type_lu</strong> - holds information about data types, like
 * String, Date,
 * Decimal etc.
 * <li><strong>command</strong> - a command maps a single request to a number of
 * different
 * queries.
 * <li><strong>command_query_xref</strong> - the actual mapping between a
 * command and n queries.
 * <li><strong>input_lu</strong> - holds the inputs for the various queries.
 * <li><strong>query_input_xref</strong> - the mapping between a query and n
 * inputs.
 ...
 *
 * <p>
 * This query gives us a list of handles and ratings where the rating is greater
 * than the input
 ...
 * <li><strong>name</strong> a name for the query, this is used as a key for the
 * resultset of
 * this query
 * <li><strong>ranking</strong> 1 if this is a ranking query, 0 if it is not. If
 * it is a ranking
 * query, another column is added to the result set containing the rank of a
 * particular row.
 * If there is a tie, all those rows that are tied get the same rank, and the
 * next non-tied
 ...
 * <li><strong>input_code</strong> a code used when specifying a particular
 * input in a query
 ...
 * <li><strong>command_group_id</strong> the id of the group this command
 * belongs to
 * </ul>
 * <br>
 * <li>
 ...
 * <li><strong>query_id</strong> is the id of the query we're associating with
 * this command
 * <li><strong>sort_order</strong> is simply a way to sort the queriess for a
 * given command,
 * each record in command_query_xref for a particular command should have a
 * distinct
 ...
 * </pre>
 * <ul>
 * <li><strong>query_id</strong> is the id of the query whose inputs were are
 * setting up
 * <li><strong>optional</strong> is a flag that allows us to set defaults for
 * this input
 * <li><strong>default_value</strong> is the default value if this input was not
 * specified
 * at execution time. it should not include the INPUT_DELIMITER specified in the
 * DataAccess.properties file.
 * <li><strong>input_id</strong> is the id of the input we are associating with
 * this query
 * <li><strong>sort_order</strong> is simply a way to sort the inputs for a
 * given query,
 * each record in query_input_xref for a particular query should have a distinct
 * value
 * for sort_order
 * </ul>
 * <br>
...
public class DataRetriever {
   ...
   * @param jdbcTemplate
   */
  public DataRetriever(DBAccessor dbAccessor, JdbcTemplate jdbcTemplate) {
  ...
  /**
   * In addition to checking correctness of the input, we're also preventing a
   * clever user who knows
   * how this system works from embedding SQL in the input. TODO If we ever need
   * to add string input
  ...
  private String runDefaultInputQuery(String defaultQueryId, Map inputs) {
    ...
    List<Map<String, Object>> rs = dbAccessor.executeQuery(jdbcTemplate,
        "SELECT text FROM query WHERE query_id=" + specialQueryId);
    if (rs.isEmpty()) {
      throw new IllegalArgumentException("Query text for query ID " + specialQueryId + " missing from DB");
    }
    String specialQuery = getString(rs.get(0), "text");

    int i, j;
    // For default input queries, all inputs are required to avoid circularity
    // hassles.
    // Thus the substitution process is not table-based (no query_input_xref entries
    // here).
    // It is assumed that inputs have already passed validation in executeCommand(),
    // which should be the case if the input resolution order is specified properly
    // in
    // query_input_xref. default inputs can not include the INPUT_DELIMITER or this
    // will fail.
    while ((i = specialQuery.indexOf(DataAccessConstants.INPUT_DELIMITER)) >= 0) {
      ...
      if (j < 0)
        throw new IllegalArgumentException("Unterminated input in default input query " + defaultQueryId);
    ...
    SqlRowSet rs2 = dbAccessor.queryForRowSet(jdbcTemplate, specialQuery);
    if (!rs2.next()) {
      throw new IllegalArgumentException("Default input query " + defaultQueryId + " did not return a value");
    }
   ...
   * <tt>Map</tt> in
   * which each key is a <tt>String</tt> representing a query name, and each value
   * is a
   * <tt>ResultSetContainer</tt> containing the data returned by that query.
   *
   * @param inputMap A map of inputs to this command. Each key in this map is a
   *                 valid input code in
   *                 DataAccess.properties, and each value is a <tt>String</tt>
   *                 containing the value passed in
   *                 for the given input code. One key-value pair must contain a
   *                 valid command description as
   *                 specified in the "command" table.
   * @return The statistical data requested by the command.
   * @throws Exception If some problem is encountered while executing the queries
   *                   specified by the
   *                   passed-in command.
   */
  public Map<String, List<Map<String, Object>>> executeCommand(Map inputMap) throws DataAccessException {
    ...
    if (commandDesc == null) {
      throw new IllegalArgumentException("Missing command description");
    }
    ...
    try {
      ...
      List<Map<String, Object>> rs = dbAccessor.executeQuery(jdbcTemplate, query.toString(), commandDesc);
      rowcount = 0;
      qid = new ArrayList();
      ...
      for (Map<String, Object> row : rs) {
        ...
        if (isRanking == 1)
          querySortMap.put(tempId, getInt(row, "column_index"));
      }
      ...
      throw new IllegalArgumentException("Invalid command: " + commandDesc);
    }

    // Now get the inputs of the queries
    try {
      if (queryIdList.length == 0) {
        throw new IllegalArgumentException("Query information for command " + commandDesc + " missing from DB");
      }
      ...
      List<Map<String, Object>> rs1 = dbAccessor.executeQuery(jdbcTemplate, query.toString(), commandDesc);
      ...
      for (i = 0; i < rowcount; i++) {
        ...
        if (input == null) {
          if (!"Y".equals(optional)) {
            throw new IllegalArgumentException("Missing required query input: " + inputCode);
          }
        ...
        if (!validateInput(input, dataType))
          throw new IllegalArgumentException("Invalid data for input " + inputCode + ": " + input);
        ...
        String old = DataAccessConstants.INPUT_DELIMITER + inputCode + DataAccessConstants.INPUT_DELIMITER;
      ...
      for (i = 0; i < queryIdList.length; i++) {
        ...
        for (int j = 0; j < rowcount; j++) {
          if (queryText.indexOf(
            ...
            throw new IllegalArgumentException("Query input entries missing from database: " + queryText);
          }
        }
      }
    ...
    try {
      ...
      throw new InternalError("Error while retrieving query data:" + queryText);
    }
  }
