...
/**
 * This class is the core of the component and actually generates the IDs. It is
 * also used by
 * IDGeneratorBean. This class queries a database to find the current state of
 * the sequence -- the
 * lowest ID not yet generated, and the current block size. It allocates to
 * itself the next
 * &quot;block&quot; of IDs and then updates the ID sequence state in the
 * database accordingly, then
 * allocates IDs from this block until exhausted. Note that larger block sizes
 * are more efficient,
 * as this class has to make fewer database updates. However also note that any
 * IDs not yet assigned
 * when this instance is destroyed are simply lost, so large block sizes may
 * also result in larger
 * &quot;gaps&quot; in the ID sequence. This class requires a
 * javax.sql.DataSource instance to be
 * made available under the JNDI name
 * &quot;java:comp/env/jdbc/com/topcoder/util/idgenerator/IDGeneratorDataSource&quot;;
 * this
 * DataSource is used to access the ID table. This class is synchronized for
 * thread-safety.
 *
 * <p>
 * Notes: the default connection name is 'DefaultSequence' for
 * DBConnectionFactory component.
 *
...
public class IDGenerator {
  ...
  /**
   * The key to the next_block_start field name of table id_sequences in
   * configuration.
   */
  ...
  /**
   * The name of the ID sequence which this instance encapsulates. This is set by
   * the constructor
  ...
  /**
   * This is the next value that will be generated for this sequence. It is
   * returned and updated by
   * getNextID().
   */
  private long nextID;
   ...
   * @throws IDGenerationException     if an error occurs while retrieving ID
   *                                   sequence configuration
   *                                   (for example, database errors)
   * @throws NoSuchIDSequenceException if name is null, or no such ID sequence is
   *                                   configured in the
   *                                   database
   */
  ...
  /**
   * Creates a new IDGeneratorImpl for the named ID sequence with the
   * configuration namespace and a
   ...
   * @throws IDGenerationException if an error occurs while generating the ID (for
   *                               example, error
  ...
  /**
   * Wraps the value that would be returned by getNextID() in a BigInteger
